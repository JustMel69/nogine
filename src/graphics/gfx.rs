...
pub fn integer_scaling_mouse_pos(screen_pos: Vector2, src_res: (u32, u32), dst_res: (u32, u32)) -> Result<Vector2, Vector2> {
    let scaling = integer_scaling::get_scaling(src_res, dst_res);
    let rect = integer_scaling::get_screen_rect(src_res, dst_res, scaling);
    ...
    let pixel_pos = Vector2(x * src_res.0 as f32, y * src_res.1 as f32);
...
pub fn screen_to_world_pos(screen_pos: Vector2, screen_res: (u32, u32)) -> Vector2 {
    let screen_pos = Vector2(screen_pos.0, screen_res.1 as f32 - screen_pos.1); // Invert y-axis
    let pos_clip = screen_pos.inv_scale(Vector2(screen_res.0 as f32, screen_res.1 as f32)) * 2.0 - Vector2::ONE;
