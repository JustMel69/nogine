use nogine::{graphics::{Graphics, pipeline::{RenderPipeline, RenderTexture, SceneRenderData, DEFAULT_RENDER_TARGET}, RenderStats, texture::TextureFiltering, gfx::integer_scaling}, window::{WindowCfg, WindowMode}, color::{Color4, Color}, math::Vector2, unwrap_res};
...
fn main() {
    // Create Window
    let mut window = unwrap_res!(WindowCfg::default().main(true).res((1280, 720)).title("Integer Scaling Example").mode(WindowMode::Windowed).init());
    ...
    while window.is_running() {
        ...
        Graphics::set_cam(Vector2::ZERO, Vector2(1.5 * 1.5, 1.5));
